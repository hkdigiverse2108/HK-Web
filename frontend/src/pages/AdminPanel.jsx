            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-5 py-4 pr-12 bg-black/40 border border-white/10 rounded-xl text-white font-sans text-sm focus:outline-none focus:border-white/30 transition-colors"
                  required
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-neutral-400 hover:text-white transition-colors cursor-pointer flex items-center justify-center"
                >
                  {showPassword ? (
                ...
                {selectedItem ? (
                  <div className="p-5 bg-white/[0.02] border border-white/5 rounded-xl space-y-4 text-left animate-fadeIn">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      ...
                      <div className="mb-4">
                        <label className="font-mono text-[8px] text-neutral-500 block mb-1">Managers (Select up to 2)</label>
                        <div className="max-h-40 overflow-y-auto border border-white/10 rounded-lg p-2 bg-black/40 space-y-1">
                          {currentContent.people
                            .filter(p => p.name !== selectedItem.name)
                            .sort((a, b) => (a.level || 0) - (b.level || 0) || a.name.localeCompare(b.name))
                            .map(p => {
                              const parentIds = String(selectedItem.parent_id || '').split(',').map(s => s.trim()).filter(Boolean);
                              const isChecked = parentIds.includes(p.name);
                              return (
                                <label key={p.name} className="flex items-center gap-2 text-xs text-neutral-300 hover:text-white cursor-pointer select-none py-0.5">
                                  <input
                                    type="checkbox"
                                    checked={isChecked}
                                    onChange={(e) => {
                                      ...
                                      if (e.target.checked) {
                                        if (parentIds.length >= 2) {
                                          alert("You can assign at most 2 managers.");
                                          return;
                                        }
                                        newParentIds = [...parentIds, p.name];
                                      } else {
                                        newParentIds = parentIds.filter(id => id !== p.name);
                                      }
                                      updatePeopleItem(selectedIndex, 'parent_id', newParentIds.join(', ') || null);
                                    }}
                                    className="rounded border-white/20 bg-black text-white focus:ring-0 focus:ring-offset-0"
                                  />
                                  <span>Lvl {p.level} - {p.name}</span>
                                </label>
                              );
                            })}
                        </div>
                      ...
                      <div className="flex gap-4 items-center pt-2">
                        {selectedItem.image && (
                          <div className="w-10 h-10 rounded-full overflow-hidden border border-white/10 shrink-0 bg-neutral-900 flex items-center justify-center">
                            <img src={resolveImageUrl(selectedItem.image)} alt="Avatar Preview" className="w-full h-full object-cover" />
                          </div>
                        )}
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex gap-2 items-stretch">
                            <input
                              ...
                              className="flex-1 px-3 py-1.5 bg-black border border-white/10 rounded-lg text-white text-xs focus:outline-none min-w-0 font-mono"
                            />
                            <label className="px-3 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 rounded-lg text-[9px] uppercase tracking-wider font-mono text-neutral-300 hover:text-white cursor-pointer flex items-center justify-center shrink-0 transition-colors">
                              Upload
                              <input
                                type="file"
                                accept="image/*"
